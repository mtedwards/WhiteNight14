...
class pb_backupbuddy_backup {
	...
	public function shutdown_function() {
		...
		$main_file = $log_directory . 'status-' . pb_backupbuddy::$options['log_serial'] . '.txt';
		...
		// Write to log.
		@file_put_contents( $main_file, $e_string );
		if ( $write_serial === true ) {
			@file_put_contents( $serial_file, $e_string );
		}
	...
	function cron_next_step( $spawn_cron = true, $future_offset = 0 ) {
		...
		pb_backupbuddy::status( 'details', 'Preparing to schedule Cron for `' . $this->_backup['serial'] . '`.' );
		...
		pb_backupbuddy::status( 'details', 'Actually scheduling cron next.' );
		$schedule_response = wp_schedule_single_event( ( time() + $future_offset ), pb_backupbuddy::cron_tag( 'process_backup' ), array( $this->_backup['serial'] ) );
		if ( $schedule_response === false ) {
			pb_backupbuddy::status( 'error', 'Cron reported error scheduling -- wp_schedule_single_event() returned false.' );
		} else {
			pb_backupbuddy::status( 'details', 'Cron scheduled.' );
		}
		if ( $spawn_cron === true ) {
			pb_backupbuddy::status( 'details', 'Spawning cron.' );
			spawn_cron( time() + 150 ); // Adds > 60 seconds to get around once per minute cron running limit.
			pb_backupbuddy::status( 'details', 'Cron spawned.' );
		}
		update_option( '_transient_doing_cron', 0 ); // Prevent cron-blocking for next item.
		
		pb_backupbuddy::status( 'details', 'cron_next_step() complete.' );
	...
	function backup_zip_files() {
		...
		$zip_response = pb_backupbuddy::$classes['zipbuddy']->add_directory_to_zip(
			...
			pb_backupbuddy::$classes['core']->get_directory_exclusions(),	// array	Files/directories to exclude. (array of strings).
