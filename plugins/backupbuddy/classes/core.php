...
class pb_backupbuddy_core {
	...
	function send_remote_destination( $destination_id, $file, $trigger = '', $send_importbuddy = false ) {
		...
		if ( $destination['type'] == 's3' ) {
			...
			if ( $destination['active_mode'] == '0' ) {
				$active_mode = false;
			} else {
				$active_mode = true;
			}
			
			$response = $this->remote_send_ftp( $destination['address'], $destination['username'], $destination['password'], $destination['path'], $destination['ftps'], $file, $destination['archive_limit'], $active_mode, $send_importbuddy );
	 ...
	 *	@param		string		$file				Full file path to the file to send.
	 *	@param		int			$limit				Maximum number of backups for this site in this directory for this account. No limit if zero 0.
	 *	@param		bool		$active				true = active mode, false = passive.
	 *	@param		bool		$send_importbuddy	Whether or not to also send importbuddy.php.
	 *	@return		bool							True on success, else false.
	 */
	function remote_send_ftp( $server, $username, $password, $path, $ftps, $file, $limit = 0, $active = true, $send_importbuddy = false ) {
	...
	function test_ftp( $server, $username, $password, $path, $type = 'ftp', $active = true ) {
	...
	function periodic_cleanup( $backup_age_limit = 43200, $die_on_fail = true ) {
		...
		pb_backupbuddy::status( 'details', 'Cleaning up leftover importbuddy.php script in site root if it exists.' );
		...
		pb_backupbuddy::status( 'details', 'Cleaning up leftover importbuddy directory in site root if it exists.' );
