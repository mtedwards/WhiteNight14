...
class pb_backupbuddy_ajax extends pb_backupbuddy_ajaxcore {
	...
	public function remote_send() {
		...
		wp_schedule_single_event( time(), pb_backupbuddy::cron_tag( 'remote_send' ), array( pb_backupbuddy::_POST( 'destination_id' ), pb_backupbuddy::$options['backup_directory'] . pb_backupbuddy::_POST( 'file' ), pb_backupbuddy::_POST( 'trigger' ), $send_importbuddy ) );
	...
	function remote_test() {
		...
		if ( pb_backupbuddy::_POST( '#type' ) == 's3' ) {
			if ( pb_backupbuddy::_POST( '#ssl' ) == '1' ) {
				$s3_ssl = true;
			} else {
				$s3_ssl = false;
			}
			if ( true === ( $response = pb_backupbuddy::$classes['core']->test_s3( pb_backupbuddy::_POST( '#accesskey' ), pb_backupbuddy::_POST( '#secretkey' ), pb_backupbuddy::_POST( '#bucket' ), pb_backupbuddy::_POST( '#directory' ), $s3_ssl ) ) ) {
				echo __('Test completed successfully.', 'it-l10n-backupbuddy' );
			} else {
				echo __('Failure', 'it-l10n-backupbuddy' ) . '; ' . $response;
			}
		} elseif ( pb_backupbuddy::_POST( '#type' ) == 'rackspace' ) {
			if ( true === ( $response = pb_backupbuddy::$classes['core']->test_rackspace( pb_backupbuddy::_POST( '#username' ), pb_backupbuddy::_POST( '#api_key' ), pb_backupbuddy::_POST( '#container' ), pb_backupbuddy::_POST( '#server' ) ) ) ) {
				echo __('Test completed successfully.', 'it-l10n-backupbuddy' );
			} else {
				echo __('Failure', 'it-l10n-backupbuddy' ) . '; ' . $response;
			}
		} elseif ( pb_backupbuddy::_POST( '#type' ) == 'ftp' ) { // FTP TEST.
			
			if ( pb_backupbuddy::_POST( '#ftps' ) == '0' ) {
				$ftp_type = 'ftp';
			} else {
				$ftp_type = 'ftps';
			}
			
			if ( pb_backupbuddy::_POST( '#active_mode' ) == '0' ) {
				$active_mode = false;
			} else {
				$active_mode = true;
			}
			
			if ( true === ( $response = pb_backupbuddy::$classes['core']->test_ftp( pb_backupbuddy::_POST( '#address' ), pb_backupbuddy::_POST( '#username' ), pb_backupbuddy::_POST( '#password' ), pb_backupbuddy::_POST( '#path' ), $ftp_type, $active_mode ) ) ) {
				echo __('Test completed successfully.', 'it-l10n-backupbuddy' );
			} else {
				echo __('Failure', 'it-l10n-backupbuddy' ) . '; ' . $response;
			}
			...
			echo 'Error #4343489. There is not an automated test available for this service `' . pb_backupbuddy::_POST( '#type' ) . '` at this time.';
		}
	...
	function exclude_tree() {
		...
		if( file_exists( $root . pb_backupbuddy::_POST( 'dir' ) ) ) {
			$files = scandir( $root . pb_backupbuddy::_POST( 'dir' ) );
			natcasesort( $files );
			if( count( $files ) > 2 ) { /* The 2 accounts for . and .. */
				echo '<ul class="jqueryFileTree" style="display: none;">';
				foreach( $files as $file ) {
					if( file_exists( $root . pb_backupbuddy::_POST( 'dir' ) . $file ) && ( $file != '.' ) && ( $file != '..' ) && ( is_dir( $root . pb_backupbuddy::_POST( 'dir' ) . $file ) ) ) {
						echo '<li class="directory collapsed"><a href="#" rel="' . htmlentities(pb_backupbuddy::_POST( 'dir' ) . $file) . '/">' . htmlentities($file) . ' <img src="' . pb_backupbuddy::plugin_url() . '/images/bullet_delete.png" style="vertical-align: -3px;" /></a></li>';
					}
				}
				echo '</ul>';
			} else {
